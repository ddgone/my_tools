...
3. **全局与焦点快捷键 (Shortcut Isolation)**：
   ...
   - `Ctrl+F`: 在主页菜单中，收藏/取消收藏当前选中的工具。
   - `Ctrl+R`: 在主页菜单中，一键恢复到上一次工作会话的工具页面。
   - `Ctrl+S`: 自动导出输出框日志到配置的导出目录，并复制到剪贴板。
   - `Ctrl+L`: 融合清理键。清空输出框日志，同时清理任务栏中所有已完成/失败的任务。
   - `Ctrl+U`: 撤销清理。不仅恢复输出日志，还会将刚刚清理掉的任务栏数据完美复原。
   - `Ctrl+B`: 切换侧边多任务管理栏的显示/隐藏状态。
