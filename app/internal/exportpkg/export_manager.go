...
func NewManager(state *shared.SharedState, dlg *dialog.Manager, ensureTooling func(*shared.SharedState) error) *Manager {
	return &Manager{state: state, dialog: dlg, ensureTooling: ensureTooling}
}
...
func (m *Manager) ExportTool(req ExportToolRequest) (*ExportToolResult, error) {
	...
	defaultDir := dialog.SanitizeDefaultDirectory(lastDir)
	if defaultDir == "" {
	...
	filePath, err := m.dialog.OpenSaveFileDialog(m.state.Ctx, dialog.FileDialogRequest{
		Title:            "导出工具",
...
func (m *Manager) emitExportProgress(toolID string, message string) {
...
func (m *Manager) OpenPath(path string) error {
...
func (m *Manager) LoadLastExportDirectory() (string, error) {
	cfg, err := m.LoadExportConfig()
	if err != nil {
		return "", err
	}
	return dialog.SanitizeDefaultDirectory(cfg.LastDirectory), nil
}

func (m *Manager) SaveLastExportDirectory(dir string) error {
	cfg, err := m.LoadExportConfig()
	if err != nil {
		return err
	}
...
func (m *Manager) LoadExportConfig() (exportConfig, error) {
	layout, err := runtimeenv.ResolveLayout()
	if err != nil {
...
func (m *Manager) WriteExportConfig(cfg exportConfig) error {
	layout, err := runtimeenv.ResolveLayout()
	if err != nil {
...
func defaultConfigDocument() map[string]json.RawMessage {
	return map[string]json.RawMessage{
		"export": json.RawMessage(`{"lastDirectory":""}`),
	}
}
